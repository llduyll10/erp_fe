...
import { useCreateTeamOrder } from "@/services/team-order";
...
export function CreateTeamOrderPage() {
  const navigate = useNavigate();
  const { mutate: create, isPending } = useCreateTeamOrder();
  ...
  const form = useForm({ resolver: zodResolver(schema), defaultValues: {
    ...
    delivery_address: "", recipient_phone: "", deposit_amount: 0, cod_amount: 0, delivery_note: "",
  }});
  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold">Tạo đơn đội</h1>
      </div>

      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          {/* Mẫu áo — full width, shows image preview when selected */}
            ...
            <FormItem>
              ...
              {selectedProduct ? (
                ...
                <div className="flex items-center gap-4 border rounded-lg p-3 bg-muted/30">
                  <div className="flex-shrink-0 w-24 h-24 rounded-md overflow-hidden border bg-white flex items-center justify-center">
                    <OptimizedImage
                      fileKey={selectedProduct.file_key}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {selectedProduct.description && (
                      <p className="text-sm text-muted-foreground mt-0.5 line-clamp-2">{selectedProduct.description}</p>
                    )}
                  </div>
                  <Button
                    type="button" variant="ghost" size="sm"
                    className="flex-shrink-0 text-muted-foreground hover:text-red-500"
                    onClick={() => { setSelectedProduct(null); field.onChange(""); }}>
                    <X size={16} className="mr-1" /> Đổi mẫu
                  </Button>
                </div>
              ) : (
                /* Selector when not yet chosen */
                <FormControl>
                  <AutocompleteSearch<Product>
                    ...
                    emptyMessage="Không tìm thấy mẫu áo"
                  />
                </FormControl>
              )}
              <FormMessage />
          ...
          <FormField control={form.control} name="contact" render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="VD: Nguyễn Trọng Minh" className="max-w-sm" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )} />
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium leading-none">Ghi chú chung</p>
                <p className="text-xs text-muted-foreground mt-1">Mỗi dòng là một yêu cầu riêng</p>
              </div>
              <Button
            ...
            <div className="space-y-2">
              {noteLines.map((line, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="text-muted-foreground font-bold text-base w-4 text-center select-none">•</span>
                  <Input
                    value={line}
                    onChange={(e) =>
                      setNoteLines((p) => p.map((l, idx) => idx === i ? e.target.value : l))
                    }
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        setNoteLines((p) => [...p.slice(0, i + 1), "", ...p.slice(i + 1)]);
                      }
                      if (e.key === "Backspace" && line === "" && noteLines.length > 1) {
                        e.preventDefault();
                        setNoteLines((p) => p.filter((_, idx) => idx !== i));
                      }
                    }}
                    placeholder={
                      i === 0 ? "VD: Có số quần" :
                      i === 1 ? "VD: Logo đội ngực trái" :
                      "VD: Logo xxx vai trái"
                    }
          ...
          <div className="grid grid-cols-2 gap-6">
            <FormField control={form.control} name="mockup_key" render={({ field }) => (
              <FormItem>
                ...
                {mockupKey && (
                  <div className="mb-2 rounded-md overflow-hidden border w-full h-48">
                    <OptimizedImage fileKey={mockupKey} alt="Mockup" className="w-full h-full object-contain" showLoading />
                  </div>
                )}
                <FormControl>
                  <ImageUpload
                    ...
                    placeholder="Tải mockup mẫu đã chốt"
                  />
                </FormControl>
                <FormMessage />
              </FormItem>
            ...
            <FormField control={form.control} name="logo_key" render={({ field }) => (
              <FormItem>
                ...
                {logoKey && (
                  <div className="mb-2 rounded-md overflow-hidden border w-full h-48">
                    <OptimizedImage fileKey={logoKey} alt="Logo" className="w-full h-full object-contain" showLoading />
                  </div>
                )}
                <FormControl>
                  <ImageUpload
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <FormLabel className="text-base">
                ...
                <span className="ml-2 text-sm font-normal text-muted-foreground">({items.length} người)</span>
              </FormLabel>
          ...
          <div className="flex gap-2">
            ...
            <Button type="submit" disabled={isPending}>
              {isPending ? "Đang tạo..." : "Tạo đơn đội"}
            </Button>
          </div>
        </form>
      </Form>
